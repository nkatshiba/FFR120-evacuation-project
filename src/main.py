...
def experiment(N=30, T=1000, R=1, D=25, eta=0.1, stepsize=2):
    ...
    fig.patch.set_facecolor('#242424')  # Set the figure background color to black
    ax.set_facecolor('#020202')  # Set the axes background color to dark gray
    ax.grid(color='#fff', linestyle='-', linewidth=0.5)  # Set the grid color to white
    ax.set_xlim(0, D)
    ax.set_ylim(0, D)
    ...
    for t in range(T-1):
        ...
        trajectory[:,t+1] = update_rule_4(trajectory[:,t], exit_point, alarm_on, stepsize=stepsize, eta=eta, R=R, D=D)
        ax.clear() # Clear the axes
        ax.set_xlim(0, D)
        ax.set_ylim(0, D)
        ax.set_title('Simulaion of an Evacuation', color='#fdb777', y=1.05)  # Set the title after clearing the axes
        ...
        ax.text(exit_point[0], exit_point[1], 'Exit', ha='center', va='bottom', color='#00A26B') # Add a label for the exit
        ax.text(0, D - 0.1, 'Exit counter: {}'.format(exit_counter), ha='left', va='top', color='#74e3b1', weight='bold', fontsize=10)
        if alarm_on:
            ax.text(D/2, 0, 'Alarm ON', ha='center', va='bottom', color='#E61C29', fontsize=14) # Display the alarm status
